...
- 键盘输入代码的四个步骤：
  ...
  3. 调用Scanner类的相关方法（`next() / nextXxx()`），来获取指定类型的变量（获取输入）
  4. 释放资源：`scan.close();`
...
1、**Math类的random()的调用**，会返回一个**[0,1)范围**的一个double型值
